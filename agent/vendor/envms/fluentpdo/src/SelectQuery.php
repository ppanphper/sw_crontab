 ...
 *                        ($statement can be 'table' name only or 'table:' means back reference)
 * @method SelectQuery  rightJoin(string $statement) add RIGHT JOIN to query
 *                        ($statement can be 'table' name only or 'table:' means back reference)
...
class SelectQuery extends CommonQuery implements Countable
{
    ...
    public function fetchColumn($columnNumber = 0) {
        if (($s = $this->execute()) !== false) {
            $result = $s->fetchColumn($columnNumber);
            $s->closeCursor();
            return $result;
        }
     ...
     *
     * @return array|bool|\PDOStatement of fetched rows
     */
    public function fetchAll($index = '', $selectOnly = '') {
        ...
        if ($index) {
            ...
            if (($s = $this->execute()) !== false) {
                $result = $s->fetchAll();
                if($this->convertTypes) {
                    $result = FluentUtils::convertToNativeTypes($s, $result);
                }
                $s->closeCursor();
                return $result;
            }
