...
abstract class BaseQuery implements IteratorAggregate
{
    ...
    public function execute()
    {
        ...
        try {
            if ($result && $result->execute($parameters)) {
                ...
                $this->fpdo->setErrorInfo([]);
            } else {
                $this->fpdo->setErrorInfo($result->errorInfo());
                $errorInfo = $result->errorInfo();
                throw new Exception('errorCode = '.$errorInfo[1].'; errorMsg = '. $errorInfo[2]);
            }
        } catch (Exception $e) {
            logWarning('[SQL Error]: ' . $e->getMessage());
    ...
    public function getQuery($formatted = true)
    {
        ...
        $query = sprintf($query, ...$parameters);
        return $query;
    }
    ...
    private function handleException(Exception $e) {
        //如果异常抛出是MySQL server has gone away
        $offset = stripos($e->getMessage(),'MySQL server has gone away');
        if($offset !== false) {
            // 重连
            return $this->fpdo->reconnect();
        }
        return false;
    }
